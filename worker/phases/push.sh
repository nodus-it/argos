...
phase_push_preconditions() {
    ...
    if ! agent_auth_present; then
        echo "push: keine Authentifizierung für Agent '${AGENT_NAME:-claude_code}' (commit-message braucht den Agent) — bitte CLAUDE_CODE_OAUTH_TOKEN (claude_code) oder ~/.codex/auth.json / OPENAI_API_KEY (codex) setzen." >&2
        return 3
    fi
    return 0
}
