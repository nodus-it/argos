<?php

declare(strict_types=1);

namespace App\Models;

use App\Enums\WorkerImageEntityStatus;
use Illuminate\Database\Eloquent\Collection;
use Illuminate\Database\Eloquent\Concerns\HasUlids;
use Illuminate\Database\Eloquent\Factories\HasFactory;
use Illuminate\Database\Eloquent\Model;
use Illuminate\Database\Eloquent\Relations\BelongsTo;
use Illuminate\Database\Eloquent\Relations\HasMany;
use Illuminate\Support\Carbon;

/**
 * @property string $id
 * @property string $name
 * @property string $label
 * @property bool $is_builtin
 * @property string $install_script
 * @property string $runner_class
 * @property string|null $npm_pkg
 * @property string|null $pinned_version
 * @property array<int, string>|null $requires_stack_capabilities
 * @property array<string, mixed>|null $config_schema
 * @property WorkerImageEntityStatus $status
 * @property string|null $installed_version
 * @property string|null $upstream_version
 * @property bool $has_update
 * @property Carbon|null $last_checked_at
 * @property Carbon|null $last_built_at
 * @property int|null $created_by_user_id
 * @property Carbon|null $created_at
 * @property Carbon|null $updated_at
 * @property-read User|null $createdBy
 * @property-read Collection<int, AgentCredential> $credentials
 * @property-read Collection<int, WorkerImageBuild> $imageBuilds
 */
class WorkerAgent extends Model
{
    use HasFactory;
    use HasUlids;

    protected $fillable = [
        'name',
        'label',
        'is_builtin',
        'install_script',
        'runner_class',
        'npm_pkg',
        'pinned_version',
        'requires_stack_capabilities',
        'config_schema',
        'status',
        'installed_version',
        'upstream_version',
        'has_update',
        'last_checked_at',
        'last_built_at',
        'created_by_user_id',
    ];

    protected function casts(): array
    {
        return [
            'is_builtin' => 'boolean',
            'requires_stack_capabilities' => 'array',
            'config_schema' => 'array',
            'status' => WorkerImageEntityStatus::class,
            'has_update' => 'boolean',
            'last_checked_at' => 'datetime',
            'last_built_at' => 'datetime',
        ];
    }

    /**
     * @return BelongsTo<User, $this>
     */
    public function createdBy(): BelongsTo
    {
        return $this->belongsTo(User::class, 'created_by_user_id');
    }

    /**
     * @return HasMany<AgentCredential, $this>
     */
    public function credentials(): HasMany
    {
        return $this->hasMany(AgentCredential::class);
    }

    /**
     * @return HasMany<WorkerImageBuild, $this>
     */
    public function imageBuilds(): HasMany
    {
        return $this->hasMany(WorkerImageBuild::class);
    }
}
