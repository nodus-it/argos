            ...
            <div x-data="{ tab: '{{ ($implementSummaryNontechnicalHtml || $implementSummaryTechnicalHtml) ? 'implement' : 'diff' }}' }">
                        ...
                        <div class="border-t border-gray-100 dark:border-gray-800 pt-4">
                            ...
                            <div class="flex flex-col gap-2">
                                @foreach($implementHistory as $entry)
                                    <div x-data="{ open: false, view: 'nontechnical' }"
                                         class="rounded-lg border border-gray-200 dark:border-gray-700 overflow-hidden">
                                        <button type="button" x-on:click="open = !open"
                                                class="w-full flex items-center justify-between px-4 py-2.5 bg-gray-50 dark:bg-gray-800/60 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors text-left">
                                            <span class="text-xs font-medium text-gray-500 dark:text-gray-400">{{ $entry['timestamp'] }}</span>
                ...
                <div x-show="tab === 'diff'" x-cloak class="p-4">
                    ...
                    @else
                        {{-- Stat bar --}}
                        @if(trim($diffStat) !== '')
                            <div class="mb-3 rounded-lg border border-slate-700 bg-slate-900 px-4 py-2.5 font-mono text-xs text-slate-300 whitespace-pre-wrap leading-5">{{ $diffStat }}</div>
                        @endif
                        ...
                        <div class="flex flex-col gap-2">
                            @foreach($diffFiles as $file)
                                <div x-data="{ open: true }"
                                     class="rounded-lg border border-slate-700 overflow-hidden bg-slate-950">

                                    {{-- File header --}}
                                    <button type="button" x-on:click="open = !open"
                                            class="w-full flex items-center justify-between px-4 py-2.5 bg-slate-900 hover:bg-slate-800 transition-colors text-left gap-3">
                                        <div class="flex items-center gap-2 min-w-0">
                                            ...
                                            @endif
                                            <span class="font-mono text-xs text-slate-300 truncate">{{ $file['to_path'] ?: $file['from_path'] }}</span>
                                        </div>
                                    ...
                                    <div x-show="open" x-collapse>
                                        <table class="w-full border-collapse font-mono text-xs leading-5">
                                            @foreach($file['hunks'] as $hunk)
                                                {{-- Hunk header --}}
                                                <tr class="bg-sky-950/30 border-t border-b border-sky-900/40">
                                                    <td class="w-12 px-2 py-1 text-slate-600 select-none text-right"></td>
                                                    <td class="w-12 px-2 py-1 text-slate-600 select-none text-right"></td>
                                                    <td class="px-3 py-1 text-sky-400/80">
                                                ...
                                                @foreach($hunk['lines'] as $dline)
                                                    <tr @class([
                                                        'border-b border-slate-800/50 group',
                                                        'bg-emerald-950/30 hover:bg-emerald-950/50' => $dline['type'] === 'add',
                                                        'bg-red-950/30 hover:bg-red-950/50' => $dline['type'] === 'del',
                                                        'hover:bg-slate-900/40' => $dline['type'] === 'context',
                                                    ])>
                                                        <td class="w-12 px-3 py-0 text-slate-600 select-none text-right align-top leading-5 pt-0.5">{{ $dline['old_num'] ?? '' }}</td>
                                                        <td class="w-12 px-3 py-0 text-slate-600 select-none text-right align-top leading-5 pt-0.5">{{ $dline['new_num'] ?? '' }}</td>
                                                        <td @class([
                                                            ...
                                                            'text-slate-700' => $dline['type'] === 'context',
